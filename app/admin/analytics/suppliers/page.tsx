...
interface SupplierAnalyticsData {
  ...
  top_sellers: Array<{ id: number; business_name: string; total_revenue: number | string }>;
  top_listers: Array<{ id: number; business_name: string; product_count: number | string }>;
  product_buckets: Record<string, number>;
  monthly_products: Array<{ month: string; count: number | string }>;
}
...
export default function SupplierAnalytics() {
  ...
  const toNumber = (val: string | number) => (typeof val === 'string' ? parseFloat(val) : val);

  // Total approved suppliers would ideally come from backend; using top_sellers as fallback
  const activeSuppliers = data.top_sellers.length; // Replace with data.total_suppliers when available

  const avgProductsPerSupplier = data.top_listers.length > 0
    ? Math.round(toNumber(data.total_products) / data.top_listers.length)
    : 0;
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <MetricCard
          ...
          value={toNumber(data.total_products)}
          ...
          <div className="space-y-3">
            {data.top_sellers.map((s, idx) => (
              <div key={s.id} className="flex items-center justify-between">
                ...
                <span className="font-bold">${toNumber(s.total_revenue).toFixed(2)}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-3">
            {data.top_listers.map((s, idx) => (
              <div key={s.id} className="flex items-center justify-between">
                ...
                <span className="font-bold">{toNumber(s.product_count)} products</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data.monthly_products.slice().reverse().map(item => ({ ...item, count: toNumber(item.count) }))}>
              <CartesianGrid strokeDasharray="3 3" />
